...
ESP32_CAM_SKETCH_DIR="$PROJECT_DIR"
DEFAULT_ESP32_FQBN="esp32:esp32:esp32cam"
DEFAULT_ESP32_PORT="/dev/ttyUSB0"
DEFAULT_UPLOAD_PROPERTIES=("upload.speed=115200")
MODE="upload"
...
usage() {
  cat <<'EOF'
Usage:
  ./esp32-cam-ov3660-streamer/upload-sketch.sh [--mode <compile|upload>] [--fqbn <fqbn>] [--port <port>]

Behavior:
  - Uses the hardcoded ESP32-CAM OV3660 streamer sketch path.
  - Default mode is `upload`.
  - `--mode compile` only compiles.
  - `--mode upload` compiles first, then uploads.
  - Default board is `esp32:esp32:esp32cam`.
  - Default port is `/dev/ttyUSB0`.
  - Default upload property is `upload.speed=115200` for safer ESP32-CAM uploads.
...
ARGS=(
  --mode "$MODE"
  --fqbn "$FQBN"
  --sketch "$ESP32_CAM_SKETCH_DIR"
)

UPLOAD_HELP_PORT=""
if [[ "$MODE" == "upload" ]]; then
  ARGS+=(--port "$PORT")
  for property in "${DEFAULT_UPLOAD_PROPERTIES[@]}"; do
    ARGS+=(--upload-property "$property")
  done
